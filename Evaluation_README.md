...
- **腳本路徑**: `E:\MAE_AST\MAE_AST\generate_pearson_graph.py`
- **輸入資料來源 (Input)**: 
  - `predictions.json` (模型推論產出的結果檔，路徑: `/home/lin/MAE_output/predictions.json`)
- **產出圖表 (Output)**:
  - `pearson_heatmap.png`: Pearson 相關係數熱力圖。
  - `pearson_network_graph.png`: Pearson 節點關係圖（NetworkX 繪製），紅線為正相關，藍線為負相關。
- **執行方式 (WSL)**:
  ```bash
  conda activate mae_ast
  python /mnt/e/MAE_AST/MAE_AST/generate_pearson_graph.py
  ```
...
- **腳本路徑**: `E:\MAE_AST\MAE_AST\verify_and_plot.py`
- **輸入資料來源 (Input)**:
  1. `train.json` (訓練集標籤，用來計算各類別的 Support/樣本數，路徑: `E:\MAE_AST\MAE_AST\mae_finetune\json\train.json`)
  2. `test.json` (測試集標籤，作為 Ground Truth 與預測結果對答案，路徑: `E:\MAE_AST\MAE_AST\mae_finetune\json\test.json`)
  3. `predictions.json` (推論結果，超過 180 萬筆推論，程式會透過檔名自動找出對應測試集的 607 筆資料，路徑: `/home/lin/MAE_output/predictions.json`)
- **產出圖表 (Output)**:
  - `support_vs_recall.png`: 散點圖。X軸為該類別在 `train.json` 的總數 (Support)，Y軸為該類別在 `test.json` 的召回率 (Threshold=0.5)。
  - `pr_curve.png`: Precision-Recall 曲線，展示各類別在不同信心門檻下的精確率與召回率變化，圖例中包含 AP (Average Precision) 分數。
- **執行方式 (WSL)**:
  ```bash
  conda activate mae_ast
  python /mnt/e/MAE_AST/MAE_AST/verify_and_plot.py
  ```
...
- 腳本的輸出圖表預設儲存在 `E:\MAE_AST\MAE_AST\` 根目錄下。
